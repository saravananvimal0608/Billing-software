  ...
  const handleSubmit = async (mode) => {
    try {
      ...
      const res = await commonApi({
        method: "POST",
        endpoint: "api/order/",
        data: { products: cart, paymentMode: mode },
      });
          ...
          <div onClick={() => setShowPayment(false)}>
            {cart.length > 0 && (
              ...
              >
                ...
                {/* for outside click showpayment hide */}
                {showPayment && (
                  <div
          ...
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map((p) => (
              <div key={p._id} className="col-10 col-md-4 col-lg-3 mb-4">
                <div className="p-3 text-center rounded shadow text-white bg-color-primary">
                  <h5>{p.productName}</h5>
                  ...
                  <div className="d-flex justify-content-center align-items-center mt-3 gap-2">
                    <button
                      className="btn btn-sm btn-light fw-bold px-3"
